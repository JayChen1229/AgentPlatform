...
| **MCP 工具掛載模式切換** | 支援 **`[ 🎯 精確工具 Deterministic ]`** 與 **`[ ⚡ 整台伺服器 Dynamic ]`** 雙模式切換 |
| **精確工具 (Deterministic)** | 選擇單一已核准 MCP 工具，唯讀展示 inputSchema 與 requiredScope，提供上游變數參數映射區塊 |
| **整台伺服器 (Dynamic)** | 一鍵選定整台已註冊 MCP 服務器，以 Function Calling 形式開放給上游 LLM 動態選用。支援唯讀可折疊已核准工具清單 |
| **權限點聯集 (Scope Union Check)** | 動態計算整台 MCP 伺服器內所有已核准工具所需權限點的聯集；若 Agent 身份缺少任一權限點，即時顯示紅字警示標籤 |
| **批次工具授權申請** | 選用整台伺服器動態模式時，可一鍵生成「批次工具授權」申請單至資安治理佇列，審核通過後方可部署上線 |
